...
We derive the state $h_{k,d}$ at time $t = k\Delta$ with time step~$\Delta$ by splitting the integration range $[0,\,k\Delta]$ into $[0,\,(k{-}1)\Delta]$ and $[(k{-}1)\Delta,\,k\Delta]$:
\begin{equation}
  h_{k,d} = \int_{0}^{(k-1)\Delta}\!\Phi_d(k\Delta,s)\,\mathbf{b}(s)\,x_d(s)\,ds
...
\begin{equation}
  h_{k,d} = \Phi_d\bigl(k\Delta,(k{-}1)\Delta\bigr)
  \underbrace{\int_{0}^{(k-1)\Delta}\!\Phi_d\bigl((k{-}1)\Delta,s\bigr)\,\mathbf{b}(s)\,x_d(s)\,ds}_{\displaystyle h_{k-1,d}}
  + \int_{(k-1)\Delta}^{k\Delta}\!\Phi_d(k\Delta,s)\,\mathbf{b}(s)\,x_d(s)\,ds
\end{equation}
...
\begin{equation}\label{eq:scalar-update}
  \underbrace{h_{k,d}}_{N\times 1}
  = \underbrace{\mathbf{A}_{k,d}}_{N\times N}\;\underbrace{h_{k-1,d}}_{N\times 1}
...
Define the state matrix $\mathbf{H}_k \in \mathbb{C}^{D\times N}$ whose $d$-th row is $h_{k,d}^\top$:
\begin{equation}
  \mathbf{H}_k
  = \begin{pmatrix} h_{k,1}^\top \\ h_{k,2}^\top \\ \vdots \\ h_{k,D}^\top \end{pmatrix}
...
\begin{equation}
  \bigl(\mathbf{A}_{k,d}\,h_{k-1,d}\bigr)_n = A_{k,d,n}\cdot h_{d,k-1,n}
\end{equation}
Transposing to row form, the $d$-th row of the transition term is:
\begin{equation}
  \bigl(\mathbf{A}_{k,d}\,h_{k-1,d}\bigr)^\top
  = \bigl(A_{k,d,1}\,h_{d,k-1,1},\;\dots,\;A_{k,d,N}\,h_{d,k-1,N}\bigr)
\end{equation}
...
\begin{equation}
  \begin{pmatrix}
    (\mathbf{A}_{k,1}\,h_{k-1,1})^\top \\
    \vdots \\
    (\mathbf{A}_{k,D}\,h_{k-1,D})^\top
  \end{pmatrix}
...
\begin{equation}
  h_{t,d} = \sum_{s=0}^{t}\Biggl(\prod_{j=s+1}^{t}\mathbf{A}_{j,d}\Biggr)\bar{\mathbf{b}}_s\,x_{s,d}
\end{equation}

\subsection*{Step 3: Substitution into the Output Equation and Kernel Extraction}

Substituting into the output $y_{t,d} = \mathbf{c}_t \cdot h_{t,d}$ and expanding componentwise using the properties of diagonal matrices:
\begin{equation}
