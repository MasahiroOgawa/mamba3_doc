...
\begin{itemize}
  ...
  \item $h(t) \in \mathbb{C}^{D \times N}$: State matrix. Each row $h_d(t) \in \mathbb{C}^{1 \times N}$ is the state vector of channel~$d$.
...
We begin with the differential equation for the state $h_d(t) \in \mathbb{C}^N$ (column vector notation) of each channel~$d$:
\begin{equation}
...
\begin{equation}\label{eq:continuous}
  ...
  \,\mathbf{b}(s)\,x_d(s)\,ds
  \qquad\bigl[h_d(t)\in\mathbb{C}^N\bigr]
\end{equation}
...
\begin{equation}\label{eq:scalar-update}
  ...
  = \underbrace{\mathbf{A}_{d,k}}_{N\times N}\;\underbrace{h_{k-1,d}}_{N\times 1}
...
\begin{equation}
  \bigl(\mathbf{A}_{d,k}\,h_{k-1,d}\bigr)_n = A_{d,k,n}\cdot h_{k-1,d,n}
\end{equation}
Transposing to row form, the $d$-th row of the transition term is:
\begin{equation}
  \bigl(\mathbf{A}_{d,k}\,h_{k-1,d}\bigr)^\top
  = \bigl(A_{d,k,1}\,h_{k-1,d,1},\;\dots,\;A_{d,k,N}\,h_{k-1,d,N}\bigr)
\end{equation}
...
\begin{equation}
  \begin{pmatrix}
    (\mathbf{A}_{1,k}\,h_{k-1,1})^\top \\
    \vdots \\
    (\mathbf{A}_{D,k}\,h_{k-1,D})^\top
  \end{pmatrix}
...
\begin{equation}
  h_{t,d} = \sum_{s=0}^{t}\Biggl(\prod_{j=s+1}^{t}\mathbf{A}_{d,j}\Biggr)\bar{\mathbf{b}}_s\,x_{s,d}
\end{equation}
...
\begin{equation}
  y_{t,d} = \sum_{s=0}^{t}\Biggl[\sum_{n=1}^{N}(c_{t,n}\cdot\bar{b}_{s,n})\prod_{j=s+1}^{t}A_{d,j,n}\Biggr]x_{s,d}
\end{equation}
