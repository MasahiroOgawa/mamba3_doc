...
(\texttt{original\_paper/mamba3.pdf}) so every equation can be
...
\texttt{mamba3.pdf}, Section~2 and Section~3.
...
\begin{equation}\label{eq:rec2}
  ...
  \bm{y}_t = \bm{C}_t^\top\,\bm{h}_t,
\end{equation}
which is \texttt{mamba3.pdf} eq.~(1). Mamba-3's \emph{parallel} form
(\texttt{mamba3.pdf} eq.~(2)) reads
...
\end{equation}
which is the Mamba-2 mask of \texttt{mamba3.pdf} eq.~(3). Using
...
\end{equation}
This is exactly \eqref{eq:ssd} (\texttt{mamba3.pdf} eq.~(2)).

\subsection{Reading \eqref{eq:ssd-derived} as linear attention}
...
\texttt{mamba3.pdf} eq.~(7): $\bm{L}$ is the product of a
...
\textbf{BCNorm} (RMSNorm on $\bm{B}_t,\bm{C}_t$; \texttt{mamba3.pdf}
...
\end{equation}
and Proposition~4 of \texttt{mamba3.pdf} shows that this is equivalent
...
Mamba-3's MIMO variant (\texttt{mamba3.pdf} \S 3.3, eqs.~(12)–(14))
...
\begin{enumerate}
  ...
  \item Apply the usual post-processing (output projection +
        ...
        Llama block (\texttt{mamba3.pdf}~Fig.~2).
\end{enumerate}

\subsection{Multi-view cross-attention for 3-D applications}
\label{sec:multiview}

For multi-view inputs (e.g.\ \texttt{depthAnything3.pdf}, Fig.~2),
...
design as \texttt{depthAnything3.pdf} Fig.~2 (``within-view
